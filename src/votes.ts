...
export const getRCVs = async (limit: number): Promise<Array<string>> => {
  ...
  let votes = await response.data.map(
    (doc: { identifier: string }) => doc.identifier
  )
...
export const getVotesFromRCV = async (
  id: string
): Promise<Array<DocumentVote>> => {
  ...
  try {
    return parseHTMLToDocumentVoteArray(id, url, text, meps.meps)
  } catch (e) {
    throw new Error(
      'Tried to query: ' + response.url + ' But got and invalid html: ' + e
    )
  }
}

export const parseHTMLToDocumentVoteArray = (
  id: string,
  url: string,
  html: string,
  meps: Array<Mep>
...
export const parseStringToHTMLArray = (html: string): Array<HTMLElement> => {
  ...
  const votes: Array<HTMLElement> = root
    .getElementsByTagName('table')
    .filter((table) => table.attributes.class === 'doc_box_header')
  return votes
}

export const parseMepNamesToID = (
  meps: Array<Mep>,
  positive: Array<string>,
  negative: Array<string>,
  abstained: Array<string>
): VoteResults => {
...
export const parseHTMLVote = (html: HTMLElement, meps: Array<Mep>): Vote => {
  ...
  if (titleSpan === undefined || titleSpan.length === 0) {
    titleID =
      titleName.match(
        /([A-Z]{1,2}-[A-Z0-9]{1,3}-[0-9]{4}\/[0-9]{4})|([A-Z][0-9]-[0-9]{4}\/[0-9]{4})/g
      )?.[0] || ''
  } else {
