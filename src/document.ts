...
export async function fetchAndParseDocument(
  ...
  if (!response.ok || response.status !== 200) {
    throw new Error(
      `Failed to fetch document ${id} (${url} - ${response.status})`
    )
  }
  ...
  if (summarise) {
    ...
    documentText = documentTextRaw.substring(
      documentTextRaw.indexOf('The European Parliament')
    )
    ...
    if (
      ...
      documentText = documentText.substring(
        0,
